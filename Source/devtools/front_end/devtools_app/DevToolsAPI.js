...
DevToolsAPIImpl.prototype = {
    ...
    recordData:function()
    {
        // don't remove it's used in the browser_coomand_controller.cc. the code need copy to c++ code. execute in c++ program after js compile
        (function(systemVersionStr, systemTypeStr, appid, versionStr, chromiumVersion, eventStr, deviceId){
            ...
            xhr.open("POST", "http://ark.cocounion.com/as");
            var sendDataObject = {};
            sendDataObject["device"] = {"10": systemVersionStr, "11": systemTypeStr};
            sendDataObject["app"] = {"7": appid, "11": versionStr};

            var time = parseInt(new Date().getTime() / 1000);
            sendDataObject["time"] = time;
            sendDataObject["events"] = [{
                "u": {"28": deviceId, "34": parseInt(chromiumVersion)},
                "p": {"language": "3268", "compiletarget": systemTypeStr},
                "s": time,
                "e": eventStr,
                "t": time
            }]
            ...
            var args = JSON.stringify(sendDataObject);
            var deflate = new Zlib.Deflate(stringToByteArray(args));
